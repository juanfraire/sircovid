...
func obstaculos(X float64, Y float64, X1 float64, Y1 float64) (float64, float64, bool) {
	...
	objetos[31] = []int{1, 74, 1, 279}
	objetos[32] = []int{1, 63, 280, 22}
	objetos[33] = []int{66, 5, 280, 12}
	objetos[34] = []int{450, 12, 391, 26}
	objetos[35] = []int{581, 11, 392, 23}
	objetos[36] = []int{454, 138, 434, 12}
	objetos[37] = []int{455, 134, 338, 13}
	objetos[38] = []int{384, 79, 0, 63}
	objetos[39] = []int{387, 71, 65, 7}
	objetos[40] = []int{410, 29, 59, 24}
	objetos[41] = []int{465, 494, 0, 77}
	objetos[42] = []int{955, 18, 1, 60}
	objetos[43] = []int{957, 10, 61, 11}
	objetos[44] = []int{998, 41, 17, 13}
	objetos[45] = []int{721, 290, 161, 94}
	objetos[46] = []int{1027, 12, 197, 26}
	objetos[47] = []int{964, 28, 339, 23}
	objetos[48] = []int{1027, 28, 340, 21}
	objetos[49] = []int{81, 191, 417, 111}
	//abajo para que no salga de la pantalla
	objetos[50] = []int{0, 0, 0, screenHeight}
	objetos[51] = []int{0, screenHeight, 0, 0}
	objetos[52] = []int{0, screenWidth, screenHeight, 32}
	objetos[53] = []int{screenWidth, 0, 0, screenHeight}
